# ===================================
# Quick wallet test & balance check
# ===================================
import os
from dotenv import load_dotenv
from web3 import Web3
from _func import CHAIN_ID
...
usdce_c = w3.eth.contract(address=USDC_E,   abi=ERC20_ABI)
...
usdce_eoa = usdce_c.functions.balanceOf(EOA).call()
...
print(f"USDC   (EOA) : {usdce_eoa / 1e6:.6f}")
...
if DEPOSIT_WALLET:
    ...
    usdce_dw = usdce_c.functions.balanceOf(DW).call()
    print(f"\nDeposit Wallet : {DW}")
    print(f"POL     (wallet) : {matic / 1e6:.6f}")
    print(f"pUSD    (wallet) : {pusd_dw / 1e6:.6f}")
    print(f"USDC.e  (wallet) : {usdc_dw / 1e6:.6f}")
    print(f"USDC    (wallet) : {usdce_dw / 1e6:.6f}")
else:
